...
public class CommandManager {
    ...
    //data model only convert the timemillis   to day ,month,hour, min
    // this method return all data from bracelet..
...
//        data[6] = (byte)0;//占位符，没意
        ...
        // broadcast data sent the command to bracelet
        broadcastData(data);
    }
...
//        data[6] = (byte)0;//占位符
     ...
     */
    // the localbroadcast manager is inbuilt function in android what does this function do?
    // it sent a service msg to ble service and then sent the byte ok wil ping you if i get any query or question, thanks ok
    private void broadcastData(byte[] bytes) {
        final Intent intent = new Intent(BleConstans.ACTION_SEND_DATA_TO_BLE); // this notify the broadcast service using a key ACTION_SEND_DATA_TO_BLE
        intent.putExtra(Constans.EXTRA_SEND_DATA_TO_BLE, bytes); // then these bytes is pass into the intent
        try {
            LocalBroadcastManager.getInstance(mContext).sendBroadcast(intent); //this is when it is sent
        } catch (Exception e) {
