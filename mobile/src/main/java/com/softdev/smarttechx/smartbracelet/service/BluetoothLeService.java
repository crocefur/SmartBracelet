...
import com.softdev.smarttechx.smartbracelet.util.SampleGattAttributes;
...
//                        SPUtils.BIND_DEVICE_ADDRESS), false);
    ...
    private void broadcastUpdate(final String action,
        ...
        // carried out as per profile specifications: // any help i can offer
...
//        SPUtils.putFloat(getApplicationContext(), SPUtils.BAND_VERSION_TYPE, 0);
    ...
    private void BLE_send_data_set(byte[] data, boolean retry_status) {
       ...
       /* final byte[] txValue = data;
        Log.i("datacalled", "Received data：" + DataHandlerUtils.bytesToHexStr(txValue));
        // am i right
        List<Integer> datas = DataHandlerUtils.bytesToArrayList(txValue);
        ArrayList<Integer> dataw = new ArrayList<>();

        Log.i("datacalled", datas.toString());
        ...
        for (int i = 0; i < dataw.size(); i++) {
            stringBuffer.append(dataw.get(i) + " ");
        }
        Log.i("datacalled", stringBuffer.toString());*/
